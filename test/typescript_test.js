...
const __1 = require("..");
const Classic = require('../src/classic');
...
describe('TypeScript support', () => {
    describe('Automerge.init()', () => {
        ...
        it('should allow the freeze option to be passed in', () => {
            let s1 = Automerge.init({ freeze: true });
        ...
        it('should allow a frontend to be `any`', () => {
            const s0 = __1.Frontend.init();
            const [s1, req1] = __1.Frontend.change(s0, doc => (doc.key = 'value'));
            assert.strictEqual(s1.key, 'value');
            assert.strictEqual(s1.nonexistent, undefined);
            assert.strictEqual(UUID_PATTERN.test(__1.Frontend.getActorId(s1)), true);
        });
        it('should allow a frontend type to be specified', () => {
            const s0 = __1.Frontend.init();
            const [s1, req1] = __1.Frontend.change(s0, doc => (doc.birds = ['goldfinch']));
            assert.strictEqual(s1.birds[0], 'goldfinch');
            assert.deepStrictEqual(s1, { birds: ['goldfinch'] });
        });
        it('should allow a frontend actorId to be configured', () => {
            const s0 = __1.Frontend.init('111111');
            assert.strictEqual(__1.Frontend.getActorId(s0), '111111');
        });
        it('should allow frontend actorId assignment to be deferred', () => {
            const s0 = __1.Frontend.init({ deferActorId: true });
            assert.strictEqual(__1.Frontend.getActorId(s0), undefined);
            const s1 = __1.Frontend.setActorId(s0, 'abcdef1234');
            const [s2, req] = __1.Frontend.change(s1, doc => (doc.number = 15));
            assert.deepStrictEqual(s2, { number: 15 });
        });
        it('should allow a frontend to be initialized with `from`', () => {
            const [s1, req1] = __1.Frontend.from({ birds: [] });
            assert.strictEqual(s1.birds.length, 0);
            const [s2, req2] = __1.Frontend.change(s1, doc => doc.birds.push('magpie'));
            assert.strictEqual(s2.birds[0], 'magpie');
        });
        it('should allow options to be passed to Frontend.from()', () => {
            const [s1, req1] = __1.Frontend.from({ birds: [] }, { actorId: '1234' });
            assert.strictEqual(__1.Frontend.getActorId(s1), '1234');
            assert.deepStrictEqual(s1, { birds: [] });
            const [s2, req2] = __1.Frontend.from({ birds: [] }, '1234');
            assert.strictEqual(__1.Frontend.getActorId(s2), '1234');
        });
        it('should allow the length of the array to be increased', () => {
            let s1 = Automerge.from({ birds: [] });
        ...
        it('should allow the length of the array to be decreased', () => {
            let s1 = Automerge.from({ birds: ['1234'] });
    ...
    describe('saving and loading', () => {
        ...
        it('should allow the actorId to be configured', () => {
            let s1 = Automerge.init();
            s1 = Automerge.change(s1, doc => (doc.birds = ['goldfinch']));
            let s2 = Automerge.load(Automerge.save(s1), '111111');
            assert.strictEqual(Automerge.getActorId(s2), '111111');
        });
        it('should allow the freeze option to be passed in', () => {
            let s1 = Automerge.init();
            s1 = Automerge.change(s1, doc => (doc.birds = ['goldfinch']));
            let s2 = Automerge.load(Automerge.save(s1), { freeze: true });
            assert.strictEqual(Object.isFrozen(s2), true);
            assert.strictEqual(Object.isFrozen(s2.birds), true);
        });
    });
    describe('making changes', () => {
        ...
        it('should allow inspection of conflicts', () => {
            ...
            let s2 = Automerge.init('222222');
            s2 = Automerge.change(s2, doc => (doc.number = 42));
            let s3 = Automerge.merge(s1, s2);
            assert.strictEqual(s3.number, 42);
            assert.deepStrictEqual(Automerge.getConflicts(s3, 'number'), { '1@111111': 3, '1@222222': 42 });
        });
        it('should allow changes in the frontend', () => {
            const s0 = __1.Frontend.init();
            const [s1, change1] = __1.Frontend.change(s0, doc => (doc.birds = ['goldfinch']));
            const [s2, change2] = __1.Frontend.change(s1, doc => doc.birds.push('chaffinch'));
            assert.strictEqual(s2.birds[1], 'chaffinch');
            assert.deepStrictEqual(s2, { birds: ['goldfinch', 'chaffinch'] });
            assert.strictEqual(change2.actor, __1.Frontend.getActorId(s0));
        ...
        it('should accept a message in the frontend', () => {
            const s0 = __1.Frontend.init();
            const [s1, req1] = __1.Frontend.change(s0, 'test message', doc => (doc.number = 1));
            assert.strictEqual(req1.message, 'test message');
            assert.strictEqual(req1.actor, __1.Frontend.getActorId(s0));
            assert.strictEqual(req1.ops.length, 1);
        });
        it('should allow empty changes in the frontend', () => {
            const s0 = __1.Frontend.init();
            const [s1, req1] = __1.Frontend.emptyChange(s0, 'nothing happened');
        ...
        it('should work with split frontend and backend', () => {
            const s0 = __1.Frontend.init(), b0 = __1.Backend.init();
            const [s1, change1] = __1.Frontend.change(s0, doc => (doc.number = 1));
            const [b1, patch1] = __1.Backend.applyLocalChange(b0, change1);
            const s2 = __1.Frontend.applyPatch(s1, patch1);
    ...
    describe('Automerge.Text', () => {
        ...
        let doc;
        beforeEach(() => {
            doc = Automerge.change(Automerge.init(), doc => (doc.text = new Automerge.Text()));
        });
        describe('insertAt', () => {
    ...
    describe('Automerge.Table', () => {
        ...
        it('allows adding row properties', () => {
            ...
            const s2 = s1; // Cast existing table to new type
            const s3 = Automerge.change(s2, doc => (doc.books.byId(id).publisher = "O'Reilly"));
        ...
        describe('standard array operations on rows', () => {
            it('returns a list of rows', () => assert.deepEqual(s1.books.rows, [ddiaWithId]));
            it('supports `filter`', () => assert.deepStrictEqual(s1.books.filter(book => book.authors.length === 1), [ddiaWithId]));
            it('supports `find`', () => {
                assert.deepStrictEqual(s1.books.find(book => book.isbn === '1449373321'), ddiaWithId);
            });
            it('supports `map`', () => assert.deepStrictEqual(s1.books.map(book => book.title), [DDIA.title]));
        });
    });
    describe('Automerge.Counter', () => {
        it('should handle counters inside maps', () => {
            const doc1 = Automerge.change(Automerge.init(), doc => {
                doc.wrens = new __1.Counter();
            });
        ...
        describe('counter as numeric primitive', () => {
            let doc1;
            beforeEach(() => {
                doc1 = Automerge.change(Automerge.init(), doc => {
                    doc.birds = new __1.Counter(3);
                });
            });
